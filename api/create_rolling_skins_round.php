...
try {
    ...
    $tournamentName = "Rolling Skins Round - " . $currentDateTime;
    $stmt = $conn->prepare("
    ...
    $matchName = "Rolling Skins Match at " . $courseName . ", " . $currentDate;
    $stmt = $conn->prepare("
    ...
    // 6. Link golfers to tournament with handicap snapshot (no team assignments for Rolling Skins)
    if ($isManual) {
        ...
        foreach ($data['players'] as $golferId) {
            ...
            $stmt->bind_param('iid', $tournamentId, $golferId, $manual);
            $stmt->execute();
        }
    } else {
